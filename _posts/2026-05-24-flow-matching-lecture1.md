...
生成模型可以生成文本, 图片, 视频, 三维模型, 蛋白质等. 但具体生成的模态暂时不是我们关心的. 因此我们先将要生成的对象统一表示为向量

$$
\mathcal{z} \in \mathbb{R}^d
$$

对某个生成对象的好坏的评价很多时候是主观的，所以解生成问题的 metric 很难定义. 因此我们将生成问题建模为生成对象符合数据分布的可能性, 因为概率相对来讲是比较好度量的. 

在概率论的语言下，解生成问题就是建模生成的对象的概率分布. 我们记作
...
我们用于训练的数据集可以看作有限数量符合分布 $p_{data}$ 的样本
...
这里的语言比较数学, 看起来有点绕, 不过我觉得其实就是 ODE 的另一种表述语言. 
...
1. 某个变量有 $_t$ 下标意味该变量（可能）和时间 $t$ 有关。
2. 如果 vector filed $u_t(x)$ 是连续可微且导数有界的，式 （8）存在唯一解。这件事在 ML 和 DL 里一般都是成立的. 
3. 这里（8）是一个 ODE，其自变量是时间 $t$，因变量 $X$ 虽然不是一个标量，但是因为自变量是标量，所以这个还是属于 ODE 而非 PDE. 
</div>
...
从而, 可以从 init distribution 中采样, 然后用 Euler method 进行演化. 这就构成了 Flow Model 的推理过程. 

![image-20260523222323160](/images/flow-matching-lecture1/image-20260523222323160.png)

## 3 Diffusion Model

ODE 的解是一个 trajectory, **Stochastic Differential Equation(SDE)** 的解则是一个 stochastic process，或者说 random trajectory. 我们用 SDE 来建模 Diffusion Model.

在 Flow model 的基础上，我们会额外定义一个 diffusion coefficient $\sigma$, 最通用的方案是将 $\sigma$ 定义为时间的函数，即

$$
\sigma:[0,1]\to \mathbb{R}_{\ge 0},\,t\mapsto \sigma_t
$$

这里 $\sigma$ 是非负的. 当 $\sigma=0$ 的时候 Diffusion model 退化回 Flow model. (13) 可以看到 $\sigma$ 控制注入的噪声 (或者说随机性) 的强度.
...
$$
   W_t-W_s\sim \mathcal{N}(\mathbf{0},(t-s)I_d)
$$
...
3. 聪明的数学家们告诉我们在深度学习的论域里这一切拥有良好的性质, 同 ODE 一样, 我们的 SDE 有唯一解. 
</div>
