...
**Note:** 某个变量有 $_t$ 下标意味该变量（可能）和时间 $t$ 有关。
</div>

<div class="notice--info" markdown="1">
**Note:** 如果 vector filed $u_t(x)$ 是连续可微且导数有界的，式 （8）存在唯一解。这件事在 ML 和 DL 里一般都是成立的。
</div>
...
<div class="notice--info" markdown="1">
**Note:** 这里（8）是一个 ODE，其自变量是时间 $t$，因变量 $X$ 虽然不是一个标量，但是因为自变量是标量，所以这个还是属于 ODE 而非 PDE。
</div>
